...
to setup
  ...
  ask turtle 1 [
    ...
    set velocity initial-velocity-1
    set mass mass1
    set acceleration 0
  ...
  ask turtle 2 [
    set angle initial-angle-2
    set velocity initial-velocity-2
    set mass mass2
    set acceleration 0
...
to go
  ...
  ask turtle 1 [
    set acceleration ((num11 + num12 + num13 * num14) / den1) / 100
    set velocity (velocity + acceleration)
    set angle (angle + velocity * (180 / PI))
    setxy ((sin angle) * pendulum-1-length) (-(cos angle) * pendulum-1-length)
  ]

  ask turtle 2 [
    set acceleration ((num21 * (num22 + num23 + num24)) / den2 ) / 100
    set velocity (velocity + acceleration)
    set angle (angle + velocity * (180 / PI))
    setxy ((sin angle) * pendulum-2-length + [xcor] of turtle 1) (-(cos angle) * pendulum-2-length + [ycor] of turtle 1)
  ]

  Ask turtles [set velocity (velocity * 0.995)]
